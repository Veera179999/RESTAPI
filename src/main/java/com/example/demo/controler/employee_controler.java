...
import com.example.demo.exception_handling.api_response;
...
public class employee_controler {
	...
	@PutMapping   ("/update/{id}")
 	public ResponseEntity<employee_payload>updateuser(@Valid @PathVariable int id, @RequestBody employee_payload emp){
		employee_payload employee=this.service.update(emp, id);
		return ResponseEntity.ok(employee);
	}
 	@DeleteMapping("/delete/{id}")
	public ResponseEntity<api_response>delete(@Valid @PathVariable int id){
		this.service.deleteusers(id);
		return new ResponseEntity<api_response>(new api_response("id deleted sucessfully",true),HttpStatus.OK);
	}
	@GetMapping("/getall")
